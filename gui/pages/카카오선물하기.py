...
try:
    from finance.channels.kakao_present import run
...
# 파일 업로드 상태 확인
files_uploaded = file1 is not None
...
if submit_button:
    if files_uploaded:
        ...
        # run 함수 실행
        run(file1_path, result_file_path)
...
def check_session_end():
    clear_temp_dir(TEMP_DIR)
    st.write("세션 종료로 TEMP 폴더가 비워졌습니다.")
    print("세션 종료로 TEMP 폴더가 비워졌습니다.")


# 세션 종료 감지용 RunOnSaveMixin 이벤트
st.session_state.clear_func = check_session_end
