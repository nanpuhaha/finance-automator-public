...
try:
    from finance.channels.lotte_on import run
...
with col1:
    order_file: UploadedFile | None = st.file_uploader(
        "매출내역(구매확정) 파일 업로드",
        type=["xlsx"],
        help="xls 파일은 xlsx로 변환 후 업로드해주세요.",
    )
with col2:
    confirm_file: UploadedFile | None = st.file_uploader(
        "차감내역(업체) 파일 업로드",
        type=["xlsx"],
        help="xls 파일은 xlsx로 변환 후 업로드해주세요.",
    )

# 파일 업로드 상태 확인
files_uploaded = order_file is not None and confirm_file is not None
...
if submit_button:
    if files_uploaded:
        ...
        # run 함수 실행
        run(order_file_path, confirm_file_path, result_file_path)
