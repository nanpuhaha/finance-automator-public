...
try:
    from finance.channels.ssf import run
...
with col1:
    file1: UploadedFile | None = st.file_uploader(
        "입점사 정산상세내역 파일 업로드",
        type=["xlsx"],
...
# 파일 업로드 상태 확인
files_uploaded = file1 is not None and file2 is not None
...
if submit_button:
    if files_uploaded:
        ...
        # run 함수 실행
        run(file1_path, file2_path, result_file_path)
